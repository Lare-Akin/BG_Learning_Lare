...
# Ensure data directory exists (for cloud hosting)
...
def load_phrases(path=PHRASES_PATH):
    try:
        ...
        # Removed problematic filter that was causing issues
...
def save_progress_to_csv(results, filepath=PROGRESS_PATH):
    try:
        ...
        CANONICAL_FIELDS = ["UserId","SessionId","ItemId","Category","English","Bulgarian",
...
def save_contribution(contribution_type, data):
    """Save user contributions to the appropriate CSV file"""
    try:
...
def contribution_section():
    """UI for contributing new content - Now in collapsible format like other sections"""
    with st.expander("🤝 Contribute Content", expanded=False):
        ...
        with st.form("contribution_form", clear_on_submit=True):
            ...
            else:  # Phrases and Conversations
...
def ensure_itemid(df_progress):
    ...
    if "ItemId" not in df_progress.columns:
        df_progress["ItemKey"] = (df_progress.get("Bulgarian","").astype(str) + "||" + df_progress.get("English","").astype(str))
        df_progress["ItemId"] = df_progress["ItemKey"].apply(lambda s: hashlib.sha1(s.encode("utf-8")).hexdigest()[:12])
    return df_progress
...
# BALANCED SAMPLER
# -----------------------------
def sample_mixed_quiz(df_words, df_phrases, df_convos, progress_df, n=10,
                      mix={"words":0.5,"phrases":0.3,"convos":0.2}, soften=0.9):
    if progress_df is None or progress_df.empty:
        pool = []
    ...
    def weighted_sample(df_source, k):
        ...
        if rows.empty:
            return []
        rows["ItemId"] = rows.apply(lambda r: make_item_id(r["Bulgarian"], r["English"]), axis=1)
...
# -----------------------------
# Bulgarian Words Section (distinctive layout)
# -----------------------------
...
# -----------------------------
# Bulgarian Phrases Section (distinctive layout)
# -----------------------------
...
# -----------------------------
# Learn From Human Conversations (distinctive layout)
# -----------------------------
with st.expander("💬 Learn From Human Conversations"):
    if df_convo.empty:
...
# -----------------------------
# QUIZ (instrumented, balanced sampler integration)
# -----------------------------
with st.expander("🎯 Custom Quiz"):
    ...
    if df.empty or not set(["English","Bulgarian"]).issubset(df.columns):
        ...
        mix_choice = st.selectbox("Quiz mix preset", ["Balanced (50/30/20)","Quick Review (70/20/10)","Phrases heavy (20/60/20)"])
        ...
        if st.session_state.review_queue:
            if st.button("Start Review Queue Quiz"):
                ...
                combined_sources = pd.concat([
                    df.assign(_src="words"),
                    df_phrases.assign(_src="phrases"),
                    df_convo.assign(_src="convos")
                ], ignore_index=True)
                for iid in st.session_state.review_queue:
                    match = combined_sources[combined_sources.apply(
                ...
                if len(items_for_quiz) < num_questions:
                    fill = sample_mixed_quiz(
                        df, df_phrases, df_convo,
                        pd.read_csv(PROGRESS_PATH) if os.path.exists(PROGRESS_PATH) else pd.DataFrame(),
                        n=num_questions - len(items_for_quiz),
                        mix=mix
                    )
                    items_for_quiz += fill
                ...
                for row in items_for_quiz[:num_questions]:
                    ...
                    distractors = combined_available[combined_available["Bulgarian"] != row.get("Bulgarian","")]["Bulgarian"].dropna()
                    ...
                    quiz_items.append({
                        ...
                        "Transliteration": row.get("Transliteration","") if "Transliteration" in row else "",
                        "Options": options,
                        "Answered": False,
                        "Correct": None,
                        "Chosen": None,
        ...
        if st.button("Start New Quiz") or st.session_state.get("quiz_length") != num_questions:
            progress_df = pd.read_csv(PROGRESS_PATH) if os.path.exists(PROGRESS_PATH) else pd.DataFrame()
            sampled = sample_mixed_quiz(df, df_phrases, df_convo, progress_df, n=num_questions, mix=mix)
            quiz_items = []
            combined_available = pd.concat([df, df_phrases, df_convo], ignore_index=True)
            for row in sampled:
                item_id = make_item_id(row.get("Bulgarian",""), row.get("English",""))
        ...
        for i, item in enumerate(st.session_state.quiz):
            ...
            if item.get("question_shown_ts") is None:
                st.session_state.quiz[i]["question_shown_ts"] = time.time()

            # Pronunciation hint line under the question (helps immediate recall)
            ...
            if st.button("Check Answer", key=f"check_{i}") and not item["Answered"]:
                ...
                response_time = st.session_state.quiz[i]["question_answered_ts"] - st.session_state.quiz[i]["question_shown_ts"]
                chosen = selected
                ...
                try:
                    ...
                    match_df = combined.loc[
                        (combined["Bulgarian"]==item["Bulgarian"]) & (combined["English"]==item["English"])
                    ]
                ...
                if category_val:
                    st.session_state.category_scores.setdefault(category_val, {"correct": 0, "total": 0})
...
with st.expander("📊 Progress Dashboard"):
    ...
    if not progress_df.empty and "Timestamp" in progress_df.columns and "Correct" in progress_df.columns:
        ...
        if not daily_accuracy.empty:
            st.line_chart(daily_accuracy.set_index("Date"))
        else:
...
# -----------------------------
# FIXED Analysis of improvement areas
# -----------------------------
    ...
    if progress_df is not None and not progress_df.empty:
        ...
        if not items.empty:
            ...
            for _, row in hard.iterrows():
                ...
                cols[1].write(f"Category: {row['sample_Category']} • Attempts: {int(row['attempts'])} • Error: {row['error_rate']:.2f}")
            ...
            cat_stats.columns = ["Category", "Items", "Avg Error Rate", "Total Correct"]
            cat_stats["Mastery %"] = 100 - (cat_stats["Avg Error Rate"] * 100)
            ...
            if not items.empty and "days_since" in items.columns and "error_rate" in items.columns:
                chart = alt.Chart(items).mark_circle(size=60).encode(
                    ...
                    tooltip=["sample_Bulgarian", "sample_English", "attempts", "error_rate", "avg_rt", "last_seen"]
        ...
        for item_id in st.session_state.review_queue:
            if not items_stats.empty:
                ...
                if not item_data.empty:
                    ...
                    queue_items.append({"ItemId": item_id, "Bulgarian": "Not found", "English": "Not found", "Error Rate": "N/A"})
            else:
                queue_items.append({"ItemId": item_id, "Bulgarian": "Unknown", "English": "Unknown", "Error Rate": "N/A"})
